...
const ExportToPDFButtonForDriver: React.FC<ExportToPDFButtonProps> = ({
  collectionName,
  ...
  const handleExportToPDF = async () => {
    ...
    try {
      ...
      // Define the new column headers (no "Action" column)
      const tableColumn = [
        "uid",
        "name",
        "email",
      ...
      pdf.text(`Drivers as of ${currentDate}`, 14, 16);

      // Add table to PDF with automatic column widths
      pdf.autoTable({
        head: [tableColumn],
        body: tableRows,
        startY: 20,
...
export default ExportToPDFButtonForDriver;
